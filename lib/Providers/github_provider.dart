...
class GithubStateNotifier extends StateNotifier<GithubState> {
  ...
  Future<bool> validateToken(String token) async {
    ...
    final response = await http.get(Uri.parse('https://api.github.com/user'), headers: headers);
  ...
  Future<bool> validateRepo(String owner, String repo, String token) async {
    ...
    final response = await http.get(Uri.parse('https://api.github.com/repos/$owner/$repo'), headers: headers);

    if (response.statusCode == 200) {
      return true; // Repo valide
    } else {
  ...
  Future<void> getRepoCommits(String owner, String repo, String token) async {
    try {
      ...
      // Récupère les commits de base
      while (url.isNotEmpty) {
        final response = await http.get(Uri.parse(url), headers: headers);

        if (response.statusCode != 200) {
          state = state.copyWith(isLoading: false, error: "Erreur: ${response.statusCode}");
          return;
        }
      ...
      state = state.copyWith(totalCommits: commits.length);
...
//---------------------------------PROVIDER------------------------------------------//
final githubServiceProvider = StateNotifierProvider<GithubStateNotifier, GithubState>((ref) {
